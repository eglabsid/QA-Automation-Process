...
class VideoPlayer(QMainWindow):
    def __init__(self):
        ...
        file_analysis_layout.addWidget(self.open_file_button)

        self.analyze_button = QPushButton("버그 분석 시작")
        self.analyze_button.clicked.connect(self.start_bug_detection)
        self.analyze_button.setEnabled(False)  # 파일 로드 전에는 비활성화
        file_analysis_layout.addWidget(self.analyze_button)

        left_panel.addLayout(file_analysis_layout)
    ...
    def open_file(self):
        ...
        try:
            ...
            if file_name:
                ...
                self.expected_sound_events_path = video_info.get('expected_sound_events_path', '')
                ...
                self.update_log("영상이 로드되었습니다. 버그 분석 시작 버튼을 눌러 분석을 시작하세요.")
    ...
    def get_video_info(self):
        ...
        # 사운드 기대 이벤트 섹션
        sound_group = QGroupBox("사운드 기대 이벤트")
        sound_layout = QVBoxLayout()
        expected_sound_path_edit = QLineEdit()
        expected_sound_path_edit.setPlaceholderText("선택 사항: expected sound events JSON 파일 경로")
        expected_sound_browse_button = QPushButton("JSON 파일 선택")

        def browse_expected_sound_file():
            file_name, _ = QFileDialog.getOpenFileName(
                dialog,
                "사운드 기대 이벤트 JSON 선택",
                "",
            ...
            if file_name:
                expected_sound_path_edit.setText(file_name)

        expected_sound_browse_button.clicked.connect(browse_expected_sound_file)
        sound_layout.addWidget(expected_sound_path_edit)
        sound_layout.addWidget(expected_sound_browse_button)
        ...
        if dialog.exec_() == QDialog.Accepted:
            return {
                ...
                'expected_sound_events_path': expected_sound_path_edit.text().strip()
            }
        return None
    ...
    def start_bug_detection(self):
        ...
        try:
            ...
            if hasattr(self, 'expected_sound_events_path') and self.expected_sound_events_path:
                video_info['expected_sound_events_path'] = self.expected_sound_events_path
